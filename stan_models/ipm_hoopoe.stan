...
data {
  ...
  array[nsites, nyears - 1, nyears] int marray_j; // [site, occasion, recapture year]
...
parameters {
  ...
  array[nsites] vector[ny_minus_1] eta_phij_raw;
...
transformed parameters {
  ...
  vector[ny_minus_1] epsilon_phij = sig_phij * epsilon_phij_raw;
  ...
  vector[nsites] zeta_phij = sig_site_phij * zeta_phij_raw;
  ...
  array[nsites] vector[ny_minus_1] eta_phij;
  ...
  array[nsites, ny_minus_1] simplex[nyears]  pr_j;
  ...
  for (s in 1:nsites) {
    ...
    eta_phij[s] = sig_sy_phij * eta_phij_raw[s];
    ...
    pr_j[s] = marray_juveniles(nyears, phij[s], phia[s], p);
...
sig_sy_im     ~ normal(0, 0.5);
  ...
  for (s in 1:nsites) {
    for (t in 1:ny_minus_1) {
      marray_j[s, t] ~ multinomial(pr_j[s, t]);
...
generated quantities {
  ...
  real var_phij_year       = square(sig_phij);
  real var_phij_siteyear   = square(sig_sy_phij);
  real icc_phij = var_phij_year / (var_phij_year + var_phij_siteyear);
